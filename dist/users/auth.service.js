...
let AuthService = class AuthService {
    ...
    async signin(email, password) {
        const [users] = await this.usersService.findUsers(email);
        if (!users) {
            throw new common_1.NotFoundException('There was an error, the user you are looking for does not exists');
        }
        const [salt, storedHash] = users.password.split('.');
        const hash = (await scrypt(password, salt, 32));
        if (storedHash !== hash.toString('hex')) {
            throw new common_1.BadRequestException('The password is incorrect');
        }
        return users;
    }
};
